...
class ReservationCreateContract < Dry::Validation::Contract
  ...
  rule(:status_up) do
    key.failure('reservation must be active when its created') if value == false
  end

  # rule(:status_up) do
  #   key.failure('book is allready reserved') if validate(max_size: 1)
  # end
end

# contract = ReservationCreateContract.new
